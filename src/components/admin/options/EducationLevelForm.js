...
import { createEducationLevelOption, updateEducationLevelOption } from '@/lib/api';
import toast from 'react-hot-toast';

export default function EducationLevelForm({ initialData, onSubmitSuccess, onCancel }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let response;
      if (formData.id) {
        response = await updateEducationLevelOption(formData.id, dataToSave);
      } else {
        response = await createEducationLevelOption(dataToSave);
      }

      if (response.success) {
        toast.success(`Education Level ${formData.id ? 'updated' : 'created'}!`, { id: toastId });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label htmlFor="level_name" className="block text-sm font-medium text-gray-700">Level Name*</label>
        <input 
          type="text" 
          name="level_name" 
          id="level_name" 
      ...
      <div className="flex justify-end space-x-3 pt-3">
        ...
        >
          {isSaving ? 'Saving...' : (formData.id ? 'Update' : 'Create') + ' Level'}
        </button>
      </div>
    </form>
  );
