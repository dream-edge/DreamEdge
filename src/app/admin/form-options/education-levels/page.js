...
import { getEducationLevelOptions, deleteEducationLevelOption } from '@/lib/api';
import toast from 'react-hot-toast';
import { PlusIcon, PencilIcon, TrashIcon } from '@heroicons/react/24/outline';
import EducationLevelForm from '@/components/admin/options/EducationLevelForm'; // To be created
...
export default function ManageEducationLevelsPage() {
  ...
  const fetchOptions = async () => {
    setIsLoading(true);
    try {
      const response = await getEducationLevelOptions(true); // Fetch all for admin
      if (response.success) {
        setOptions(response.data);
      } else {
        toast.error(response.error || 'Failed to load education levels.');
      }
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this education level?')) {
      ...
      try {
        const response = await deleteEducationLevelOption(id);
        if (response.success) {
          toast.success('Education level deleted!', { id: toastId });
          fetchOptions();
        } else {
  ...
  if (isLoading) {
    return <div className="p-6 text-center">Loading education levels...</div>;
  }

  return (
    <div className="p-4 sm:p-6">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        <h1 className="text-xl sm:text-2xl font-semibold text-gray-800">Manage Education Levels</h1>
        <button
        ...
        >
          <PlusIcon className="h-5 w-5 mr-2" /> Add New Education Level
        </button>
      </div>
              ...
              <li key={option.id} className="px-4 py-4 sm:px-6 hover:bg-gray-50 transition-colors duration-150">
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-indigo-600 truncate">{option.level_name}</p>
      ...
      <Modal isOpen={showFormModal} onClose={() => { setShowFormModal(false); setEditingOption(null); }}>
        <h2 className="text-xl font-semibold mb-4 text-gray-800">
          {editingOption?.id ? 'Edit' : 'Add New'} Education Level
        </h2>
        <EducationLevelForm 
